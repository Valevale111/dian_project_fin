    ...
    if (decoder_init(fname) == 0)
    {
        ...
        while(true){
            if (!paused) // 如果未暂停，则获取下一帧并处理
            {
                ...
                else if (key == 'd') // 如果用户按下 'd' 键
                {
                    accelerated = !accelerated; // 切换加速状态
                }
                printf("key:%c,paused:%d,accelerated:%d\n",key,paused,accelerated);
